        ...
        <>
                    ...
                    return (
                        <div key={event.slug} className={`max-w-[1400px] mx-auto px-6 md:px-12 w-full flex flex-col py-10 md:py-16 ${index !== portfolioEvents.length - 1 ? 'border-b border-[var(--color-border-hairline)]' : ''}`}>
                            <div className="flex flex-col gap-1 mb-8 md:mb-12">
                                <Link href={lang === 'en' ? `/en/portfolio/${event.slug}` : `/portfolio/${event.slug}`} className="group w-fit">
                                ...
                                <Link href={lang === 'en' ? `/en/portfolio/${event.slug}` : `/portfolio/${event.slug}`} className="text-[13px] font-medium text-[var(--color-text-muted)] hover:text-[var(--color-text-main)] transition-colors underline underline-offset-4 mt-2 w-fit">
            ...
            <section className="py-24 md:py-32 bg-[#DDE7F0] text-[var(--color-text-main)] flex justify-center text-center">
                <div className="max-w-[800px] w-full px-6 flex flex-col gap-10 items-center">
                    ...
                    <Link href={lang === 'de' ? "/kontakt" : "/en/contact"} title="Request event photography" className="bg-[var(--color-text-main)] text-white px-10 py-4 text-base font-semibold transition-colors hover:bg-[var(--color-accent-hover)] mt-6 rounded-2xl">
                        {t.home.finalCta.btn}
                    </Link>
                </div>
            </section>
